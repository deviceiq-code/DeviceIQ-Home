...
BIN_DIR="/var/www/html/dpk"                            # subpasta onde os .bin serão servidos
BASE_URL="http://server.dts-network.com:8082"          # URL pública (host:porta)
BIN_URL_PREFIX="${BASE_URL}/dpk"                       # prefixo público da subpasta /bin
UPDATE_JSON_LOCAL="./update-dpk.json"                  # update.json na pasta atual (modelo/alvo)
PERM="0644"                                            # -rw-r--r--
### ===========================
...
ARTIFACT="${PROJECT_NAME}-${NEW_VER}.dpk"
...
OLD_JSON="$WEB_ROOT/update-dpk.json"
if $SUDO test -f "$OLD_JSON"; then
  set +e
  OLD_VER="$($SUDO jq -r '."Version" // empty' "$OLD_JSON" 2>/dev/null)"
  set -e
  if [[ -n "${OLD_VER:-}" ]]; then
    echo "[INFO] Versionando update-dpk.json atual -> $WEB_ROOT/update-dpk.json.${OLD_VER}"
    $SUDO mv "$OLD_JSON" "$WEB_ROOT/update-dpk.json.${OLD_VER}"
  else
    TS="$(date +%Y%m%d-%H%M%S)"
    echo "[WARN] Não consegui ler 'Version' do update-dpk.json atual; salvando como backup com timestamp."
    $SUDO mv "$OLD_JSON" "$WEB_ROOT/update-dpk.json.bak-${TS}"
  fi
else
  echo "[INFO] Nenhum update-dpk.json atual em $WEB_ROOT (primeira publicação?)"
fi
...
DEST_JSON="$WEB_ROOT/update-dpk.json"
...
echo "  URL:           $NEW_URL"
echo "  SHA256:        $NEW_SHA"
echo "  update-dpk.json -> $DEST_JSON"
